...
Harness entry point: the `fit` class -- fit(data, start_date_predict, end_date_predict)
then .test() returns predicted volatilities over the test days, aligned to the
project's 22-day trailing rolling-vol benchmark.
"""
...
def forward_realized_vol(prices, horizon: int = 1, annualize: bool = True) -> np.ndarray:
    """Annualized realized vol over the NEXT `horizon` days (alternative target).
    Small horizons are noisier; ~5-10 days balances noise vs. predictability."""
    ret = np.concatenate([[np.nan], daily_returns(prices)])
    fwd = pd.Series(ret ** 2).rolling(horizon).sum().shift(-horizon)
    scale = (BUSINESS_DAYS_PER_YEAR / horizon) if annualize else (1.0 / horizon)
    return np.sqrt(scale * fwd.to_numpy())
...
def learn_params(train_prices, target, feature_window: int = 1000, dt: float = DT,
    ...
    Kernel parameters live inside the features, so R1/Sigma are rebuilt every
    step -- a non-convex fit whose starting point is `theta0`.
    """
...
def predict_volatility(params: PDVParams, prices) -> np.ndarray:
    """Predicted volatility from a price series, aligned to prices."""
...
# --- harness interface ------------------------------------------------------
class fit:
    """PDV model wrapper for the model-comparison harness.

    Parameters
    ----------
    data : yfinance DataFrame for one ticker (OHLC...), spanning history from
        well before `start_date_predict` through `end_date_predict`.
    start_date_predict, end_date_predict : test window (half-open [start, end),
        matching yfinance's date convention so the test calendar lines up with
        the project's rolling_volatility benchmark).
    feature_window : PDV kernel look-back in days (paper uses 1000 ~ 4 years;
        auto-shrunk if the training history is shorter).
    target_window : realized-vol window used as the training target; defaults to
        22 to match the project's benchmark definition.
    """
    ...
    def __init__(self, data, start_date_predict, end_date_predict,
        ...
        train_prices = self.prices[self.dates < start]
        if train_prices.size == 0:
            raise ValueError("No training rows before start_date_predict; give earlier data.")

        # keep the kernel look-back short enough to leave usable training rows
        self.feature_window = int(min(feature_window,
                                      max(30, train_prices.size - target_window - 2)))
    ...
    def test(self) -> np.ndarray:
        """Predicted volatility for each test day (chronological, one per day)."""
        preds = predict_volatility(self.params, self.prices)   # over full series
        out = preds[self.test_mask]
